...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $patient) {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (empty($name)) {
            ...
            try {
                // Check if email already exists (if provided, excluding current patient)
                if (!empty($email)) {
                    $checkEmailStmt = $pdo->prepare("SELECT id FROM patients WHERE email = :email AND id != :id");
                ...
                // Check if phone already exists (if provided, excluding current patient)
                if (empty($error) && !empty($phone)) {
                    $checkPhoneStmt = $pdo->prepare("SELECT id FROM patients WHERE phone = :phone AND id != :id");
                    $checkPhoneStmt->bindParam(':phone', $phone, PDO::PARAM_STR);
                    $checkPhoneStmt->bindParam(':id', $id, PDO::PARAM_INT);
                ...
                if (empty($error)) {
                    ...
                    $updateStmt = $pdo->prepare("
                        UPDATE patients
                        SET name = :name, gender = :gender, birthdate = :birthdate, phone = :phone, email = :email, address = :address, status = :status, updatedAt = NOW()
                        WHERE id = :id
                    ");
    ...
    <?php if ($patient): ?>
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <form method="POST" action="index.php?page=patients/edit&id=<?php echo htmlspecialchars($patient['id']); ?>">
                            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">

                            <div class="mb-3">
                                <label for="name" class="form-label">Patient Name <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="name" name="name" value="<?php echo htmlspecialchars($patient['name']); ?>" required maxlength="255">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control" id="birthdate" name="birthdate" value="<?php echo htmlspecialchars($patient['birthdate'] ?? ''); ?>">
                            </div>

                            <div class="mb-3">
                                <label for="phone" class="form-label">Phone Number</label>
                                <input type="tel" class="form-control" id="phone" name="phone" value="<?php echo htmlspecialchars($patient['phone'] ?? ''); ?>" maxlength="32">
                                <div class="form-text">Enter the patient's phone number (max 32 characters)</div>
                            </div>

                            <div class="mb-3">
                                <label for="email" class="form-label">Email Address</label>
                                <input type="email" class="form-control" id="email" name="email" value="<?php echo htmlspecialchars($patient['email'] ?? ''); ?>" maxlength="64">
                            ...
                            <div class="mb-3 form-check">
                                <input type="checkbox" class="form-check-input" id="status" name="status" value="1" <?php echo $patient['status'] ? 'checked' : ''; ?>>
